...
class Attributes(ASTBase):
    ''' AST node for representing class and field attributes.
     ...
     - `[[writable]]` class attribute, triggers generation of writers and views
       ...
       (1) a legacy `X_view` struct using `utils::input_stream`, and
...
def _generate_variant_skip_deserialize(hout, typ, fragmented=False):
    ...
    using X_fragmented_view types for fragmented_temporary_buffer::istream.
    """
...
def _generate_view_struct(cout, cls, fragmented=False):
    ...
    X_fragmented_view using fragmented_temporary_buffer::istream.
    """
    members = get_members(cls)
    if fragmented:
...
def add_view(cout, cls):
    ...
    # Generate the fragmented view for fragmented_temporary_buffer::istream
