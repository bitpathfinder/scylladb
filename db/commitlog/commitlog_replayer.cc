...
detail::commitlog_entry_serialization_format db::commitlog_replayer::impl::get_entry_format(const commitlog::descriptor& d) const {
    if (d.ver >= commitlog::descriptor::segment_version_5) {
        return detail::commitlog_entry_serialization_format::variant;
    }
    return detail::commitlog_entry_serialization_format::mutation;
}

future<> db::commitlog_replayer::impl::process(stats* s, detail::commitlog_entry_serialization_format entry_format, commitlog::buffer_and_replay_position buf_rp) const {
    ...
    try {
        ...
        if (not (std::holds_alternative<raft_commit_log_entry>(read_entry) || std::holds_alternative<mutation_entry>(read_entry))) {
            rlogger.warn("Unknown type in commitlog entry, current replay position {}, ", rp);
            co_return;
        }

        if (std::holds_alternative<raft_commit_log_entry>(read_entry)) {
            const auto& raft_entry = std::get<raft_commit_log_entry>(read_entry);
