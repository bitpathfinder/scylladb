...
const std::optional<tablet_replica>* tablet_map::get_pending_replica(const tablet_id tablet) const {
    auto&& info = get_tablet_transition_info(tablet);
...
private:
    ...
    const absl::flat_hash_map<sstring, replication_factor_index_t> _datacenter_map;
    // The `_tablet_to_dc_replication_factor_map` stores replication factors
    // per data center for each Tablet.
    const utils::chunked_vector<replication_factor_list> _tablet_to_dc_replication_factor_map;
...
private:
    ...
    const tablet_replica_set& get_endpoints_for_reading(const tablet_id tablet) const {
        auto&& replicas = get_tablet_map().get_replicas_for_reading(tablet);
        return replicas;
    }
...
public:
    ...
    tablet_effective_replication_map(table_id table,
                                     ...
                                     absl::flat_hash_map<sstring, replication_factor_index_t>& datacenter_map,
                                     utils::chunked_vector<replication_factor_list>& rf_map)
    ...
    [[nodiscard]] size_t get_replication_factor(const dht::token id, const seastar::sstring& datacenter) const override {
        ...
        if (tablet.id >= _tablet_to_dc_replication_factor_map.size()) [[unlikely]] {
            on_internal_error(tablet_logger,
                    format("tablet id [{}] is out of range of replication factor map max: [{}].", tablet.id, _tablet_to_dc_replication_factor_map.size() - 1));
        }
        ...
        if (dc_it->second >= dc_rf_map.size()) [[unlikely]] {
            on_internal_error(tablet_logger, format("Data center's: [{}] index: [{}] is higher than the number of data centers in the dc_rf_map: [{}].",
    ...
    [[nodiscard]] size_t get_replication_factor(const dht::token id) const override {
        ...
        if (tablet.id >= _tablet_to_dc_replication_factor_map.size()) {
            on_internal_error(tablet_logger, format("Tablet id: [{}] index is out of _tablet_to_dc_replication_factor_map, size: [{}]", tablet.id,
                                                     _tablet_to_dc_replication_factor_map.size()));
        }
        const auto& dc_rf_map = _tablet_to_dc_replication_factor_map[tablet.id];
    ...
    virtual inet_address_vector_topology_change get_pending_endpoints(const token& search_token) const override {
        auto&& tablets = get_tablet_map();
        auto tablet = tablets.get_tablet_id(search_token);
        const auto* pending_replica_ptr = tablets.get_pending_replica(tablet);
        if (pending_replica_ptr == nullptr || !(pending_replica_ptr->has_value())) {
            return {};
        }
        const auto& pending_replica = *(*pending_replica_ptr);
        tablet_logger.trace("get_pending_endpoints({}): table={}, tablet={}, replica={}", search_token, _table, tablet, pending_replica);
        return {_tmptr->get_endpoint_for_host_id(pending_replica.host)};
    }

    inet_address_vector_replica_set get_endpoints_for_reading(const token& search_token) const override {
        const auto tablet = get_tablet_map().get_tablet_id(search_token);
        const auto& replicas = get_endpoints_for_reading(tablet);
...
// in the tablet_to_dc_replication_factor_map.
[[nodiscard]] absl::flat_hash_map<sstring, tablet_effective_replication_map::replication_factor_index_t> build_datacenter_map(const auto& data_centers_list) {
    uint32_t datacenter_index = 0;
    absl::flat_hash_map<sstring, tablet_effective_replication_map::replication_factor_index_t> datacenter_map;
...
// Populate the `tablet_to_dc_replication_factor_map` with replication factors for each data center.
...
future<utils::chunked_vector<tablet_effective_replication_map::replication_factor_list>> build_dc_replication_factor_map(const tablet_map& tablets,
        const topology& topo, const absl::flat_hash_map<sstring, tablet_effective_replication_map::replication_factor_index_t>& datacenter_map) {
    utils::chunked_vector<tablet_effective_replication_map::replication_factor_list> tablet_to_dc_replication_factor_map;
...
future<effective_replication_map_ptr> tablet_aware_replication_strategy::do_make_replication_map(
    ...
    auto erm = seastar::make_shared<tablet_effective_replication_map>(
            table, std::move(rs), std::move(tm), replication_factor, datacenter_map, replication_factor_map);
