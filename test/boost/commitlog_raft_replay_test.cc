...
// In the new implementation, any entry with idx <= last_idx and term <= last_term
// (including what were formerly "consecutive duplicates") is classified as out_of_order
// and causes filter_entries to stop processing immediately.
...
// In the new implementation, there is no explicit max_term skipping. Instead,
// old-term entries that appear after higher-indexed entries are caught by the
// out_of_order check (idx <= last_idx with same or lower term), which stops
// processing. This correctly handles the case where an old segment is replayed
// after a new one.
...
// In the new implementation, non-consecutive "duplicates" (entries from an old term
// that appear after entries from a higher-indexed new term) are caught by the
// out_of_order check: their idx <= last_idx, so processing stops immediately.
...
// After acquire_replay_position_handles_for() returns clones, destroying the
...
SEASTAR_TEST_CASE(test_clone_shared_ownership) {
    return cl_test([](commitlog& log) -> future<> {
        ...
        std::vector<replay_position> rps;
        auto handles2 = persistence.acquire_replay_position_handles_for(entries);
        for (auto& h : handles2) {
