...
async def test_unfinished_writes_during_shutdown(request: pytest.FixtureRequest, manager: ManagerClient) -> None:
    # Test for a shutdown timeout due to a stuck topology transition in `barrier_and_drain`` (see https://github.com/scylladb/scylladb/issues/23665#issuecomment-2935421243)
    # 1. Create a keyspoace with replication factor 3
    # 2. Start 3 servers
    ...
    # 6. Now with error injection, make sure another node will pause before sending a write acknowledgment.
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 3};") as ks:
        ...
        await target_server_log.wait_for("Enabling injection one-shot  \"pause_before_barrier_and_drain\"")
        ...
        logger.info("Waiting for topology change to start")
        ...
        await inject_error_on(manager, "storage_proxy_response_pause", [server_to_pause])
        logger.info(
        ...
        # Start background shutdown of the query coordinator
        ...
        await target_server_log.wait_for("Stop transport: starts")
        # Give some time for storage_service_drain_on_shutdown to work, so the tcp listeners will be closed.
        await asyncio.sleep(1)
        # Unpause the coordinator to allow shutdown
        await manager.api.message_injection(target_server.ip_addr, 'pause_before_barrier_and_drain')

        # Unpause the write response
        logger.info(f"Unblocking writes on the node {server_to_pause[2]}")
        await manager.api.message_injection(server_to_pause.ip_addr, 'storage_proxy_response_pause')
